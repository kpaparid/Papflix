...
class Movie():
    ...
    def search_movie_json(self, name, year, folder, similarity):        
        ...
        while len(filename) > 1 and continueWhile:
                ...
                for i in range(len(json['results'])):
                    if i <= 10 :
                        ...
                        if('release_date' in r and r['release_date']!="" and r['vote_count']>0):
                            ...
                            if any(x in r_name.rsplit(' ', 1) for x in [' ii', ' iii', ' iv', 'v', ' vi', ' vii', ' viii']):
                                print('REPLACING ROMANS')
                                r_name = r_name.replace(' ii', ' 2').replace(' iii', ' 3').replace(' iv', ' 4').replace(' v', ' 5').replace(' vi', ' 6').replace(' vii', ' 7').replace(' viii', ' 8')
                            ...
                            if ':' in r_name:
                                s_name = r_name.split(':')
                                s1_distance = (self.score(name, s_name[0].lower()) - 25 * count_split - (i + 1)*(count_split + 1) * 10 )* 75/100
                                s2_distance = self.score(name, s_name[1].lower()) - 25 * count_split - (i + 1)*(count_split + 1) * 10 
            ...
            if len(filename) == 1: continueWhile = False
                ...
                if count_split == 3: continueWhile = False
                if filename[0] == 'the': continueWhile = False
                # print('=======================================>split '+str(filename[0]))
            if len(filename)>1 and filename[1] != '':
                    self.scrap += filename[1]+', '
            if (len(filename) == 1 or continueWhile == False) and year != '':
                count_split = 0
                print('yOn')
                continueWhile = True
                filename = [og_name, '']
            ...
            if (len(filename) == 1 or continueWhile == False) and year == '' and folder != 'fOn':
                print('fOn')
    ...
    def get_json(self, name, year, folder, duration):
        """This module does blah blah."""
        print(name)
        js = self.search_movie_json(name, year, folder, 1, 0)
        resultNr = 0
    ...
    def download_files(self):
        ...
        self.download_image(self.id, 'https://image.tmdb.org/t/p/w600_and_h900_bestv2/'+self.backdrop_path, 'backdrop')
        i = 0
        ...
        for stars_p in sp:
            if i <= 10:
                ...
                if stars_p == 'null': 
                    ...
                    completeName = os.path.join(dir_path, str(i))
                    shutil.copy('C://Users//Marmi//Desktop/unknown.png', dir_path +str(i)+'.png')    
                else:
                    self.download_image(str(self.id) + '//stars_posters', 'https://image.tmdb.org/t/p/w600_and_h900_bestv2/'+stars_p, i)
    def download_image(self, id, url, name):
        ...
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        completeName = os.path.join(dir_path, str(name) + '.png')  

        if not os.path.exists(completeName):
            print('doesnt exist '+completeName)     
